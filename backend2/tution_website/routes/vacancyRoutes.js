...
router.get('/featured', async (req, res) => {
  try {
    ...
    .select('title subject description requirements salary _id featured status location class time')
    .lean();
...
// Add the available-vacancies route before any parameter routes
router.get('/available-vacancies', async (req, res) => {
    try {
        console.log('Fetching available vacancies (open and closed)...');
        const vacancies = await Vacancy.find({ 
            status: { $in: ['open', 'closed'] } // Include both open and closed vacancies
        })
        .select('title subject description requirements salary applications status location class time')
        .lean();

        console.log(`Found ${vacancies.length} vacancies (open and closed)`);
        res.json({
            success: true,
            data: vacancies
        });
...
// Add applicant manually by admin
router.post('/:vacancyId/admin-add-applicant', adminAuth, async (req, res) => {
    try {
        const { vacancyId } = req.params;
        const { fullName, phone, email = 'manual@entry.com' } = req.body;
        ...
        console.log(`Admin manually adding applicant to vacancy ${vacancyId}:`, { fullName, phone });
        ...
        // First check if a teacher with this phone number already exists
        let teacher = await mongoose.model('Teacher').findOne({ phone });
        
        // If teacher doesn't exist, create a temporary one
        if (!teacher) {
            console.log('Creating temporary teacher record for manual applicant');
            ...
            teacher = await mongoose.model('Teacher').create({
                ...
                password: 'manualentry' + Date.now(), // Random password
                ...
                subjects: ['Manual Entry'], // Default subject
                agreementAccepted: true,
                status: 'pending',
                cv: 'https://placeholder.com/cv.pdf', // Placeholder CV URL
            });
            
            console.log('Created temporary teacher:', teacher._id);
        } else {
            console.log('Found existing teacher with phone number:', teacher._id);
        }

        // Check if this teacher has already applied to this vacancy
        const alreadyApplied = vacancy.applications.some(app => 
            app.teacher.toString() === teacher._id.toString()
        );

        if (alreadyApplied) {
            return res.status(400).json({
                success: false,
                message: 'This teacher has already applied to this vacancy'
            });
        }
        ...
        // Return success with teacher and application data
        res.status(201).json({
            success: true,
            message: 'Applicant added successfully',
            data: {
                application: vacancy.applications[vacancy.applications.length - 1],
                teacher: {
                    _id: teacher._id,
        ...
        console.error('Error adding applicant manually:', error);
        res.status(500).json({
            success: false,
